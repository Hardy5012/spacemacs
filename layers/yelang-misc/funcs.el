...
(defun yelang/insert-semicolon-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (insert ";")))

(defun yelang/delete-semicolon-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (if (looking-back ";")
...
(defun yelang/insert-comma-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (insert ",")))

(defun yelang/delete-comma-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (if (looking-back ",")
...
(defun yelang/load-my-layout ()
  (interactive)
  (persp-load-state-from-file (concat persp-save-dir "yelang")))

(defun yelang/save-my-layout ()
  (interactive)
  (persp-save-state-to-file (concat persp-save-dir "yelang")))
...
(defun yelang/indent-region(numSpaces)
  (progn
...
(defun yelang/tab-region (N)
  (interactive "p")
  (if (use-region-p)
      (yelang/indent-region 4)               ; region was selected, call indent-region
    (insert "    ")                   ; else insert four spaces as expected
    ))

(defun yelang/untab-region (N)
  (interactive "p")
  (yelang/indent-region -4))

(defun yelang/hack-tab-key ()
  (interactive)
  (local-set-key (kbd "<tab>") 'yelang/tab-region)
  (local-set-key (kbd "<S-tab>") 'yelang/untab-region)
  )

;; I'm don't like this settings too much.
;; (add-hook 'prog-mode-hook 'yelang/hack-tab-key)
...
(defun yelang/browser-refresh--chrome-applescript ()
  (interactive)
  (do-applescript
   (format
    "
...
(defun yelang/open-file-with-projectile-or-counsel-git ()
  (interactive)
  (if (yelang/git-project-root)
      (counsel-git)
...
(defun yelang/growl-notification (title message &optional sticky)
...
(defun yelang/growl-timer (minutes message)
  ...
  (run-at-time (* (string-to-number minutes) 60)
               ...
               (lambda (minute message)
                 (yelang/growl-notification "Emacs Reminder" message t))
               minutes
               message))

(defun yelang/goto-match-paren (arg)
...
(defun yelang/hidden-dos-eol ()
  "Do not show ^M in files containing mixed UNIX and DOS line endings."
  (interactive)
  (setq buffer-display-table (make-display-table))
  (aset buffer-display-table ?\^M []))

(defun yelang/remove-dos-eol ()
  "Replace DOS eolns CR LF with Unix eolns CR"
  (interactive)
  (goto-char (point-min))
  (while (search-forward "\r" nil t) (replace-match "")))

(defun yelang/insert-chrome-current-tab-url()
  "Get the URL of the active tab of the first window"
  (interactive)
  (insert (yelang/retrieve-chrome-current-tab-url)))

(defun yelang/retrieve-chrome-current-tab-url()
  "Get the URL of the active tab of the first window"
  (interactive)
...
(defun yelang/single-lines-only ()
...
(defun yelang/named-term (name)
  (interactive "sName: ")
  (ansi-term "/bin/zsh" name))


(defun yelang/ash-term-hooks ()
...
(defun yelang/word-count-for-chinese ()
  "「較精確地」統計中/日/英文字數。
- 文章中的註解不算在字數內。
...
(defun yelang/evil-quick-replace (beg end )
  (interactive "r")
...
(defun yelang/iterm-shell-command (command &optional prefix)
  "cd to `default-directory' then run COMMAND in iTerm.
With PREFIX, cd to project root."
  (interactive (list (read-shell-command
                      "iTerm Shell Command: ")
                     current-prefix-arg))
  (let* ((dir (if prefix (yelang/git-project-root)
                default-directory))
...
(defun yelang/my-mc-mark-next-like-this ()
  (interactive)
  (if (region-active-p)
...
(defun my-open-file-in-external-app (file)
  ...
  (interactive)
  (let ((default-directory (yelang/git-project-root))
        (file-path file))
    (if file-path
        (cond
...
(defun yelang/magit-visit-pull-request ()
...
(defun yelang/search-in-fireball ()
  (interactive)
  (helm-do-ag (expand-file-name "~/Github/fireball/")))


(defun yelang/show-current-buffer-major-mode ()
  (interactive)
  (describe-variable 'major-mode))

(defun yelang/counsel-imenu ()
  (interactive)
  (counsel-imenu)
  (evil-set-jump))
